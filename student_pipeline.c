...
VertexIndex gpu_computeGLVertexID(
  ...
  /// Indexing je aktivní, pokud buffer indexů není NULL.
	if (!indices) return vertexShaderInvocation;

	else return indices[vertexShaderInvocation];
}
...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  return (void const*)((size_t)head->buffer + head->offset + gl_VertexID * head->stride);
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  {
	  if (puller->heads[i].enabled) output->attributes[i] = gpu_computeVertexAttributeDataPointer(&puller->heads[i], id);

	  else output->attributes[i] = NULL;
  }
}
...
///  - GPUVertexShaderInput()
  ...
  for (unsigned i = 0; i < nofPrimitiveVertices; ++i)
  {
	  ...
	  in.gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation + i);
	  vertexShader(&(primitive->vertices[i]), &in, gpu);
  }
...
void gpu_runFrustumPlaneClippingOnEdge(float *const minT, float *const maxT,
  ...
  *maxT          = 1.f;
  ...
  float const M  = -vertexB->data[3] + aw - s * (vertexB->data[axis] - ai);
...
void gpu_writeClippedTriangle_OneVertexVisible(
  ...
  copy_Vec3(dstCoords, srcCoords + vertex);
  ...
  copy_Vec4(dstPositions, srcPositions + vertex);
...
void gpu_runFrustumPlaneClippingOnTriangle(
  ...
  if (plane == LEFT || plane == RIGHT);
    ...
    axis          = 2;
  size_t positive = 0;
  if (plane != RIGHT && plane != TOP && plane != FAR && (plane == LEFT || plane == BOTTOM || plane == NEAR))
    positive = 1;

  float tMin[VERTICES_PER_TRIANGLE], tMax[VERTICES_PER_TRIANGLE];
  unsigned short nextV = 0;
  for (unsigned short v = 0; v < VERTICES_PER_TRIANGLE; ++v) {
	if (++nextV < VERTICES_PER_TRIANGLE)
		...
		gpu_runFrustumPlaneClippingOnEdge(
			tMin + v, tMax + v, &triangle->positions[v],
			&triangle->positions[0], axis, positive);
  }

  // Binary mask of visible vertices
  uint32_t visible = 0u;
  for (unsigned short i = 0; i < VERTICES_PER_TRIANGLE; ++i)
...
void gpu_restrictLineBorders(float *const minX, float *const maxX,
  ...
  float const d = -edgeLine->data[1] * y - edgeLine->data[2];
  if (a > 0.f)
    *minX = fmaxf(*minX, d / a);
  else if (a < 0.f)
    *maxX = fminf(*maxX, d / a);
...
void gpu_computeLineBorders(float *const minX, float *const maxX, float const y,
  ...
  assert(minX);
  assert(maxX);
  assert(triangleLines);
  *minX = -INFINITY;
  *maxX = +INFINITY;
  for (unsigned short edge = 0; edge < EDGES_PER_TRIANGLE; ++edge)
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  */
  const float xmx3 = pixelCenter->data[0] - vertices[2].data[0];//x-x3
  const float ymy3 = pixelCenter->data[1] - vertices[2].data[1];//y-y3

  const float x1mx3 = vertices[0].data[0] - vertices[2].data[0];//x1-x3
  const float y1my3 = vertices[0].data[1] - vertices[2].data[1];//y1-y3

  const float x2mx3 = vertices[1].data[0] - vertices[2].data[0];//x2-x3
  const float y2my3 = vertices[1].data[1] - vertices[2].data[1];//y2-y3

  const float rev = 1.f / (x1mx3 * y2my3 - x2mx3 * y1my3);

  coords->data[0] = (xmx3 * y2my3 - x2mx3 * ymy3) * rev;
  coords->data[1] = (x1mx3 * ymy3 - xmx3 * y1my3) * rev;
  coords->data[2] = 1.f - coords->data[1] - coords->data[0];

  /*
  coords->data[0] = (xmx3 * y2my3 - x2mx3 * (pixelCenter->data[1] - vertices[2].data[1])) / (x1mx3 * y2my3 - x2mx3 * (vertices[0].data[1] - vertices[2].data[1]));
  coords->data[1] = (pixelCenter->data[0] - vertices[2].data[0] - coords->data[0] * x1mx3) / x2mx3;
  coords->data[2] = 1.f - coords->data[1] - coords->data[0];
  */
}
...
void gpu_computeTriangleLines(Vec3       lines[EDGES_PER_TRIANGLE],
  ...
  assert(lines);
  assert(vertices);
  unsigned short nextVertex = 0;
  for (unsigned short vertex = 0; vertex < VERTICES_PER_TRIANGLE; ++vertex) {
	if (++nextVertex < VERTICES_PER_TRIANGLE)
		construct2DLine(lines + vertex, vertices + vertex, vertices + nextVertex);
	else construct2DLine(lines + vertex, vertices + vertex, vertices);
  }
}
...
float gpu_noperspectiveInterpolate(
  ...
  float result = values[0] * weights[0];
  for (unsigned short i = 1; i < WEIGHTS_PER_BARYCENTRICS; ++i)
...
float gpu_smoothInterpolate(
  ...
  for (unsigned short i = 0; i < WEIGHTS_PER_BARYCENTRICS; ++i) {
	homogenous = 1.f / homogeneousCoords[i];
    dividend += values[i] * weights[i] * homogenous;
    divisor += weights[i] * homogenous;
  }
...
void gpu_createFragment(GPUFragmentShaderInput *const fragment,
  ...
  for (unsigned short attribute = 0; attribute < MAX_ATTRIBUTES; ++attribute) {
...
void gpu_clampFragmentColor(
  ...
  for (unsigned short i = 0; i < 4; ++i) {
    ...
    else if (fragmentShaderOutput->color.data[i] > 1.f)
...
void gpu_rasterizeTriangle(GPU const gpu, GPUPrimitive const *const primitive,
  ...
  float yMin = primitive->vertices[0].gl_Position.data[1];
  float yMax = yMin;
  for (size_t v = 1; v < primitive->nofUsedVertices; ++v) {
  ...
  size_t yMaxI = gpu_roundUpPixelCoord(yMax + PIXEL_CENTER);
  ...
  for(size_t y = gpu_roundDownPixelCoord(yMin); y < yMaxI; ++y){
    ...
    xMaxI = gpu_roundUpPixelCoord  (xMax);
    if(xMaxI>=width)xMaxI = width;
    for(size_t x = gpu_roundDownPixelCoord(xMin); x < xMaxI; ++x){
